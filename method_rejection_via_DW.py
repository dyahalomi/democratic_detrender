...
def test_DW_per_epoch(
    ...
    for ii in range(0, len(x_split)):
        ...
        for jj in range(0, 10000):
            ...
            if no_pre_transit:
                DWstat_fake_pre_transit = 2.0
            else:
                fake_residuals_pre_transit = np.random.normal(
                    loc=0.0,
            ...
            if no_post_transit:
                DWstat_fake_post_transit = 2.0
            else:
                fake_residuals_post_transit = np.random.normal(
                    loc=0.0,
            ...
            DWfake = np.sqrt(
                (DWstat_fake_pre_transit - 2.0) ** 2.0
                + (DWstat_fake_post_transit - 2.0) ** 2.0
            )
        ...
        sigma = np.sqrt(2.0) * erfcinv(n_DWfake_greater_than_DWdetrend / 10000.0)
        sigmas.append(sigma)

    return sigmas


def reject_epochs_by_DW(
    x_detrended,
    y_detrended,
    yerr_detrended,
    mask_detrended,
