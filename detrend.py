		...
		for ii in range(0, len(t0s)):
			...
			if len(epoch_jump_times) == 2 : # if we have two jump times in this epoch
				...
				if t0 < epoch_jump_times[0]: # t0 is before first jump time
					...
					jump_end = find_nearest(xs,epoch_jump_times[0]) # last datapoint is the jumptime
				elif t0 > epoch_jump_times[0]: # t0 is after first jump time 
					jump_start = find_nearest(xs, epoch_jump_times[0]) #keep all data after first jumptime
					jump_end = xs[-1] # keep all data after transit
				else: # t0 = jump time... this shouldn't happen so raise an exception
					raise Exception('Your problem time appears to be mislabeled for epoch ' + str(ii+1) + '. Go back and relabel.')
					return None
...
def detrend_variable_methods(x_epochs, y_epochs, yerr_epochs, mask_epochs, mask_fitted_planet_epochs, problem_times,
	...
	if 'local' in detrend_methods:
		...
		plot_individual_outliers(local_x, local_detrended, local_x_no_outliers, local_detrended_no_outliers,
								 t0s, period, float(6*duration/(24.))/period, 0.009, save_to_directory+'local_outliers.pdf')
	...
	if 'polyAM' in detrend_methods:
		...
		plot_individual_outliers(local_x, poly_detrended, poly_x_no_outliers, poly_detrended_no_outliers,
								 t0s, period, float(6*duration/(24.))/period, 0.009, save_to_directory+'polyAM_outliers.pdf')
	...
	if 'GP' in detrend_methods:
		...
		plot_individual_outliers(local_x, gp_detrended, gp_x_no_outliers, gp_detrended_no_outliers,
								 t0s, period, float(6*duration/(24.))/period, 0.009, save_to_directory+'GP_outliers.pdf')
	...
	if 'CoFiAM' in detrend_methods:
		...
		plot_individual_outliers(local_x, cofiam_detrended, cofiam_x_no_outliers, cofiam_detrended_no_outliers,
								 t0s, period, float(6*duration/(24.))/period, 0.009, save_to_directory+'CoFiAM_outliers.pdf')
...
def detrend_sap_and_pdc(sap_values, pdc_values, save_dir, pop_out_plots, detrend_methods):
	...
	detrend_variable_methods(x_epochs = sap_values[0], y_epochs = sap_values[1], yerr_epochs = sap_values[2],
		...
		save_to_directory = save_dir+'sap_', show_plots = pop_out_plots, detrend_methods = detrend_methods)
